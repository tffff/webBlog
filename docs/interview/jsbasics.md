...
- `forEach` 不能使用 `break`、`continue`、和 `return` 配合使用

> 数据量大时：for> foreach（性能）​；日常开发中：foreach> for（可读、安全）



- `forEach`  本质是 语法糖 + Iterator
- 每次循环会有：
 - 方法调用（`hasNext()`、`next()`）
 - 额外的边界检查
- `for`是直接索引访问，CPU 更友好
...
###  14、前端跨系统通信有哪些方式？
  ...
  - 登录态共享:`cookie`
  - 页面跳转传参:`URL Hash / Query`,
  - 跨设备通信:`WebSocket`
  - 微前端子应用:`qiankun props / store`

### 15、前端登录localStorage缓存怎么设置三天自动清除?
  存时间戳 + 校验
### 16、前端浏览器缓存有哪些方式？
  - `localStorage`：永久存储永久存储，同源共享，容量 ≈ 5MB，同步 API
    - 用途：`token`、用户偏好、轻量数据
  - `sessionStorage`:标签页关闭即消失,容量 ≈ 5MB，同步 API
    - 用途：临时存储，如购物车、登录状态等
  - `cookie`:~4KB，同步 API
    - 用途：登录态、`SessionId`等
  - `IndexedDB`:容量大（几十 MB～GB）,异步,支持索引、事务,类似前端数据库
    - 用途：离线数据、富文本、大列表等
